...
<div class="hotel-results">
	<div class="hotel-details my-3 d-md-flex justify-content-between" v-for="hotel in hotels" :class="{'active': (hotel.id == active_hotel_id) }">
...
<script>
	export default {
		...
		methods: {
		    getAllWishlists(hotel_id) {
		    	this.$emit('get_all_wishlists',hotel_id);
		    },
		    removeFromWishlist(hotel_id) {
		    	this.$emit('remove_from_wishlist',hotel_id);
		    },
		}
	}
</script>