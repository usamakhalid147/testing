...
class ReportController extends Controller
{
    ...
    public function index()
    {
        $this->view_data['filter_list'] = array(
            [
                'name' => 'users',
                'display_name' => Lang::get('admin_messages.users'),
                'title' => ['User Id','Created On','Full Name','User Email','User Mobile Number','City','State','Country','Date of birth','Gender'],
                'columns' => ['user_id','created_on','full_name','user_email','user_mobile_number','city','state','country','date_of_birth','gender'],
            ],
            [
                'name' => 'hotels',
        ...
        return view('admin.reports.view',$this->view_data);
    }
    ...
    public function fetchReport(Request $request)
    {
        ...
        if($category == 'transactions') {
            $result = \App\Models\Transaction::query();
        }
        ...
        if($category == 'hotels') {
            $result = \App\Models\Hotel::query();
        }

        if($category == 'host') {
            $result = \App\Models\User::where('user_type','host')->query();
        }

        if($category == 'reservations') {
            $result = \App\Models\Reservation::with('user','host_user');
        }
        
        if($category == 'guest_cancel_reservations') {
            $result = \App\Models\Reservation::with('user','host_user')->where('status','Cancelled')->where('cancelled_by','Guest');
        }

        if($category == 'hotel_cancel_reservations') {
            $result = \App\Models\Reservation::with('user','host_user')->where('status','Cancelled')->where('cancelled_by','Host');
        }
     ...
     */
    protected function mapUsersData($users)
    {
        return $users->map(function($user) {
            return [
                'user_id' => $user->id,
                'created_on' => $user->created_at->format(DATE_FORMAT.' '.TIME_FORMAT),
                'full_name' => $user->full_name,
                'user_email' => $user->email,
                'user_mobile_number'=> $user->phone_code.' '.$user->phone_number,
     ...
     * @return \Illuminate\Support\Collection
     */
    protected function mapHostData($users)
    {
        return $users->map(function($user) {
            return [
                'user_id' => $user->id,
                ...
                'email' => $user->email,
                'status' => $user->status,
                'registered_at' => getDateInFormat($user->created_at),
            ];
        });
    }
     ...
     * @return \Illuminate\Support\Collection
     */
    protected function mapTransactionsData($transactions)
    {
        return $transactions->map(function($transaction) {
            return [
                'id' => $transaction->id,
                'list_type' => $transaction->list_type,
                'type' => $transaction->type,
                'payment_method' => $transaction->payment_method,
                'transaction_id' => $transaction->transaction_id,
                'user_name' => $transaction->user->first_name,
                'amount' => $transaction->currency_symbol.$transaction->amount,
                'created_at' => getDateInFormat($transaction->created_at),
            ];
        });
    }
